...
func (h *handler) renderTrackerClubs(w http.ResponseWriter, r *http.Request, errorMessages ...string) {
	...
	trackerClubs := make([]models.ClubWithEvents, 0, len(clubs))
	for _, club := range pinnedClubs {
		...
		trackerClubs = append(trackerClubs, models.NewPinnedClubWithEvents(clubs[i]))
		clubs = append(clubs[:i], clubs[i+1:]...)
	}

	for _, club := range clubs {
		trackerClubs = append(trackerClubs, models.NewClubWithEvents(club))
	}
