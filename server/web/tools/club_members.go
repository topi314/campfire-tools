...
func (h *handler) getEventCreators(ctx context.Context, clubID string) ([]models.Member, error) {
	members, err := h.DB.GetClubEventCreators(ctx, clubID)
	if err != nil {
		return nil, err
	}

	eventCreators := make([]models.Member, len(members))
	for i, m := range members {
		eventCreators[i] = models.NewMember(m, clubID, 32)
	}
