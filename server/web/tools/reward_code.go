...
func (h *handler) TrackerRewardCode(w http.ResponseWriter, r *http.Request) {
	...
	if code.ReservedAt != nil {
		reservedUntil = code.ReservedAt.Add(time.Minute)
	}

	if err = h.Templates().ExecuteTemplate(w, "tracker_reward_code.gohtml", TrackerRewardCodeVars{
		Reward:          models.NewReward(*reward),
		RewardCode:      models.NewRewardCode(code.RewardCode, code.ImportedByUser, code.RedeemedByUser, code.ReservedByUser),
		...
		ReserveURL:      fmt.Sprintf("/tracker/rewards/%d/codes/%d/reserve", id, codeID),
...
func (h *handler) TrackerRewardCodeDelete(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("HX-Location", fmt.Sprintf("/tracker/rewards/%d", id))
}
